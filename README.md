...
备份 schema v4 同时包含投稿分类与标签、审核记录、审核邮件任务、用户和 Key 哈希；恢复工具继续兼容 schema v1/v2/v3 旧备份。每天创建备份后会立即执行恢复预检，验证通过后才清理过期验证码、会话和限流记录。下架只改变目录状态，不删除历史包。
...
- v0.5.1 已补齐桌宠分类、标签、搜索、排序、分页和独立详情链接
