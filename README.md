...
> 首个公网版本只开放桌宠浏览和 Skill 安装。线上 `/admin`、管理 API 与投稿接口关闭；已审核目录随代码发布，桌宠包保存在 EdgeOne Blob。
...
- EdgeOne 存储：公开预览图走 CDN，桌宠 ZIP 保存在 Blob
...
- React 19 + Next.js 16
- Tencent Cloud EdgeOne Makers + Pages Blob
- vinext + Cloudflare D1/R2（仅保留给本地审核原型）
- Node.js 22+
...
公开站点默认运行在 `http://localhost:3001`。需要调试原有本地审核和投稿流程时使用 `npm run dev:registry`。
...
- EdgeOne Blob `pet-packages`：ZIP 包，网页没有直链，仅由 Skill API 读取

新增桌宠时先在本地完成审核，再上传 ZIP 到 Blob、更新目录和预览图，并运行完整测试后部署。
...
- EdgeOne 中国大陆/全球（含中国大陆）的平台预览链接只有短期有效；正式开放必须绑定自定义域名
- 管理后台和投稿未接入身份认证，因此首个公网版本关闭相关路由
- “官方 Skill”请求头只是通道约定，不是强身份认证
- 自动化测试覆盖构建和发布契约；EdgeOne Blob 的公网下载仍需部署后烟测
