...
- 版本化发布：稳定桌宠 ID 下保留不可变版本，可下架、恢复或切回旧版
...
- `registry/catalog.json`：稳定 ID、状态、当前版本、历史版本和状态记录
- `lib/public-pet-catalog.ts`：只解析并公开 `published` 的当前版本
...
下架只改变目录状态，不删除历史包；恢复时选择已存在的版本重新公开。

## 本地发布工具

所有生产命令要求 Git 工作区干净，并且必须显式传入 `--deploy`。先用
`--dry-run` 检查计划，不会修改本地或线上状态。

```powershell
# 预览发布计划
node scripts/pet-release.mjs publish --pet C:\path\to\pet --id <稳定桌宠ID> --version 1.1.0 --dry-run

# 校验、打包、上传 R2、部署并自动烟测
node scripts/pet-release.mjs publish --pet C:\path\to\pet --id <稳定桌宠ID> --version 1.1.0 --deploy

# 下架但保留全部历史版本
node scripts/pet-release.mjs unpublish --id <稳定桌宠ID> --reason "版权复核" --deploy

# 恢复或切回指定版本
node scripts/pet-release.mjs restore --id <稳定桌宠ID> --version 1.0.0 --reason "复核完成" --deploy
```

发布成功后应立即提交 `registry/catalog.json` 和新增预览图。部署流程会自动检查首页、
Skill 页面、公开目录、管理路由隔离、下载请求头以及每个当前版本的 R2 包哈希。
