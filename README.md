...
账户迁移文件为 `drizzle/0002_user_accounts.sql`，审核通知迁移文件为 `drizzle/0003_review_notifications.sql`。应用迁移前应先创建并验证一次最新 R2 备份。
...
- D1：投稿、审核事件、审核邮件任务、用户、浏览器会话和 Key 哈希
...
备份 schema v3 同时包含投稿、审核记录、审核邮件任务、用户和 Key 哈希；恢复工具继续兼容 schema v1/v2 旧备份。下架只改变目录状态，不删除历史包。
...
- v0.4.4 已支持跨设备“我的投稿”和审核结果邮件通知
- 创作者主页、改邮箱和账户恢复仍在后续版本
