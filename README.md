...
备份 schema v3 同时包含投稿、审核记录、审核邮件任务、用户和 Key 哈希；恢复工具继续兼容 schema v1/v2 旧备份。每天创建备份后会立即执行恢复预检，验证通过后才清理过期验证码、会话和限流记录。下架只改变目录状态，不删除历史包。
...
- v0.4.5 已补齐备份验证、数据清理、定时烟测和公开运营规则
